...
/* ---- storage ------------------------------------------------------------- */
const K = {
  prog:    "acca.recall.bt.progress.v1",   /* per-card schedule */
...
function savePrefs() {
  write(K.prefs, {
    ...
    size: state.size, diff: state.diff, mixups, bests, trapScores,
...
/* ==========================================================================
   Spaced repetition — Leitner boxes, in days
   ========================================================================== */
const BOX = [0, 1, 3, 7, 16, 35];

function statusOf(id) { return progress[id] || null; }
function boxOf(id) { const s = statusOf(id); return s ? s.box : -1; }

function recordGrade(id, ok, mode) {
  const s = progress[id] || { box: 0, due: 0, seen: 0, missed: 0 };
  s.seen++;
  if (ok) s.box = Math.min(s.box + 1, BOX.length - 1);
  else { s.box = 0; s.missed++; }
  s.due = today() + BOX[s.box];
  progress[id] = s;
  write(K.prog, progress);
  const card = CARD_BY_ID[id];
  if (card) logAnswer(card.c, ok, mode || state.mode);
}

/* A chapter is "mastered" in proportion to how far its cards have climbed. */
function chapterMastery(n) {
  const cards = BY_CH[n] || [];
  if (!cards.length) return 0;
  let sum = 0;
  cards.forEach(c => { const b = boxOf(c.id); if (b > 0) sum += b / (BOX.length - 1); });
...
function capMastery(cap) {
  const cards = CARDS.filter(c => CAP_OF[c.c] === cap);
  if (!cards.length) return 0;
  let sum = 0;
  cards.forEach(c => { const b = boxOf(c.id); if (b > 0) sum += b / (BOX.length - 1); });
  return sum / cards.length;
...
function buildSession(n, pool) {
  const src = pool || CARDS;
  const due = src.filter(c => statusOf(c.id) && statusOf(c.id).due <= today())
...
function cardShell(c, shown, headMeta, footHTML) {
  return '<section class="panel card-shell' + (shown ? " is-flipped" : "") + '" id="cardShell">' +
    '<div class="panel-hd"><h3>Ch ' + pad2(c.c) + ' · ' + esc(CHAPTERS[c.c]) + '</h3>' +
...
const GRADE_FOOT =
  '<button class="btn grade-again" data-grade="0">Not yet</button>' +
  '<button class="btn grade-ok" data-grade="1">Got it</button>' +
  '<span class="sp" style="flex:1"></span>' +
  '<span class="hand">press 1 or 2</span>';
...
function flipOpen(onGrade, extraFoot) {
  const shell = $("#cardShell");
  if (!shell || shell.classList.contains("is-flipped")) return;
  turnCard(shell, true);
  $("#cardFt").innerHTML = (extraFoot || "") + GRADE_FOOT;
  $$("[data-grade]").forEach(b => b.addEventListener("click", () => onGrade(b.dataset.grade === "1")));
...
function renderToday() {
  ...
  stage.innerHTML =
    ...
    '<div class="metrics">' +
      ...
      metricCard("In scope", pool.length, '<span class="hint">' + seen + ' met so far</span>') +
    '</div>' +

    panel("Session size", "cards per run",
      '<div class="chips">' + sizes.map(n =>
        '<button class="chip" data-size="' + n + '" aria-pressed="' + (state.size === n) + '">' + n + '</button>').join("") +
      '</div>' +
      '<p class="hint" style="margin-top:10px">Due cards first, oldest schedule first, then cards you have not met. ' +
      'A card you get right moves up a box (' + BOX.slice(1).join(", ") + ' days); a card you miss resets to tomorrow.</p>' +
      '<div style="margin-top:12px"><button class="btn btn-primary" id="startSession"' +
        (pool.length ? "" : " disabled") + '>Start ' + state.size + '-card session</button></div>') +
...
function renderSessionCard() {
  ...
  const grade = ok => { recordGrade(c.id, ok, "today"); ok ? s.right++ : s.wrong++; s.i++; s.shown = false; render(); };
  const reveal = () => { s.shown = true; flipOpen(grade); };

  stage.innerHTML =
    '<div class="scopebar"><h2>Today’s session</h2>' +
      '<span class="chip" style="cursor:default">' + (s.i + 1) + ' of ' + s.cards.length + '</span>' +
      '<span class="sp"></span>' +
      '<span class="hint">' + s.right + ' got · ' + s.wrong + ' to revisit</span>' +
      '<button class="btn btn-sm" id="endSession">Finish early</button>' +
    '</div>' +
    bar(pct(s.i, s.cards.length)) +
    '<div style="height:12px"></div>' +
    cardShell(c, s.shown, KIND[c.k] + ' · box ' + (st ? st.box : 0) + ' of ' + (BOX.length - 1),
      s.shown ? GRADE_FOOT
  ...
  $$("[data-grade]").forEach(b => b.addEventListener("click", () => grade(b.dataset.grade === "1")));
  ...
  keyHandler = e => {
    ...
    else if (s.shown && e.key === "1") grade(false);
    else if (s.shown && e.key === "2") grade(true);
  };
}
...
function renderSessionDone() {
  ...
  stage.innerHTML =
    ...
    '<div class="metrics">' +
      ...
      metricCard("Got it", s.right, '<span class="hint">moved up a box</span>') +
      metricCard("Missed", s.wrong, '<span class="hint">back tomorrow</span>') +
    '</div>' +
    panel("Next", "",
...
function renderCards() {
  const pool = poolNow();
  if (!state.session || state.session.kind !== "browse") {
    state.session = { kind: "browse", cards: pool.slice(), i: 0, shown: false };
  }
  const s = state.session;
  if (s.cards.length !== pool.length) { s.cards = pool.slice(); s.i = 0; s.shown = false; }
  const c = s.cards[s.i];

  if (!c) {
    stage.innerHTML = scopeBar("Cards") +
      panel("Card bank", "empty",
        emptyState("No cards in this chapter.", "Clear the chapter filter to see all " + CARDS.length + " of them."), "flush");
    wireScope();
    return;
  }
  ...
  const st = statusOf(c.id);
  const navFoot =
    '<button class="btn btn-sm" id="prev"' + (s.i === 0 ? " disabled" : "") + '>← Back</button>' +
    '<button class="btn btn-sm" id="next"' + (s.i >= s.cards.length - 1 ? " disabled" : "") + '>Next →</button>';
  const go = d => { s.i = Math.max(0, Math.min(s.cards.length - 1, s.i + d)); s.shown = false; render(); };
  const grade = ok => { recordGrade(c.id, ok, "cards"); go(1); };
  const reveal = () => { s.shown = true; flipOpen(grade, navFoot); };
  ...
  stage.innerHTML =
    ...
    bar(pct(s.i, s.cards.length)) +
    '<div style="height:12px"></div>' +
    cardShell(c, s.shown, KIND[c.k] + ' · ' + (st ? "box " + st.box + " of " + (BOX.length - 1) : "new card"),
      (s.shown ? navFoot + GRADE_FOOT
        : navFoot +
          '<button class="btn btn-primary" id="reveal">Show the answer</button>' +
          '<span class="sp" style="flex:1"></span>' +
  ...
  keyHandler = e => {
    if (e.key === " " || e.key === "Enter") { e.preventDefault(); s.shown ? go(1) : reveal(); }
...
function renderKeys() {
  const sets = {
    overview: [],
    today: [["Space", "turn the card"], ["1", "not yet"], ["2", "got it"]],
    cards: [["Space", "turn the card"], ["← →", "move"], ["1 / 2", "grade"]],
